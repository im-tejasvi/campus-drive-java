  ...
  public void run() {
    ...
    try {
      ...
      Connection conn = DriverManager.getConnection(
        "jdbc:mysql://localhost/drive?characterEncoding=UTF-8&useSSL=false",
        "root",
        "aj123"
      );
      Statement smt = conn.createStatement();
      ResultSet rs = smt.executeQuery(
        "SELECT * FROM user_creds WHERE user_id='" +
        this.user_id +
        "' AND " +
        "password='" +
        this.password +
      ...
      if (rs.next()) {
        smt.executeUpdate(
          "INSERT INTO session values(0,'" + this.user_id + "');"
        );
        rs =
          smt.executeQuery(
            "SELECT * FROM session WHERE user_id='" + this.user_id + "';"
          );
        rs.next();
